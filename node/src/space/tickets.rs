...
enum TicketWireFormat {
    Variant0(ProgramTicket),
}

impl Ticket for ProgramTicket {
    const KIND: &'static str = "prog";
...
impl FromStr for ProgramTicket {
...
impl ProgramTicket {
    ...
    pub fn into_parts(self) -> (NodeAddr, Hash, BlobFormat) {
        let ProgramTicket { node, hash, format } = self;
        (node, hash, format)
    }
}

impl Serialize for ProgramTicket {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        if serializer.is_human_readable() {
            serializer.serialize_str(&self.to_string())
        } else {
            let ProgramTicket { node, format, hash } = self;
            (node, format, hash).serialize(serializer)
        }
    }
}

impl<'de> Deserialize<'de> for ProgramTicket {
...
//         let ticket2: ProgramTicket = postcard::from_bytes(&bytes).unwrap();
...
//         let ticket2: ProgramTicket = serde_json::from_str(&json).unwrap();
//         assert_eq!(ticket2, ticket);
//     }
