...
pub fn run() {
    ...
    let (node, state) = tauri::async_runtime::block_on(async move {
        ...
        let state = AppState::open_or_create(path, &node).await.expect("failed to open app state");
    ...
    tauri::Builder::default()
        ...
        .manage(Arc::new(node))
        .invoke_handler(tauri::generate_handler![spaces_list, current_space, current_space_set, events_search, users_list, programs_list, program_run, program_get, tables_list, table_get, rows_query])
...
async fn users_list(
    ...
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            ...
            space.users().list(offset, limit).await.map_err(|e| e.to_string())
        })
    })      
}
...
async fn current_space(
    ...
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = node.spaces().get(&state.current_space_id).await.ok_or("space not found")?;
            Ok(space.details())
        })
    })
}
...
async fn current_space_set(
    ...
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = node.spaces().get(&space_id).await.ok_or("space not found")?;
...
#[tauri::command]
async fn programs_list(node: tauri::State<'_, Arc<Node>>, space_id: Uuid, offset: i64, limit: i64) -> Result<Vec<Program>, String> {
    let spaces = node.spaces().clone();
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(&space_id).await.ok_or("space not found")?;
...
#[tauri::command]
async fn program_get(node: tauri::State<'_, Arc<Node>>, space_id: Uuid, program_id: Uuid) -> Result<Program, String> {
    let spaces = node.spaces().clone();
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(&space_id).await.ok_or("space not found")?;
...
#[tauri::command]
async fn program_run(node: tauri::State<'_, Arc<Node>>, space_id: Uuid, _author: &str, program_id: Uuid, environment: HashMap<String, String>) -> Result<TaskOutput, String> {
    let spaces = node.spaces().clone();
    let node = node.clone();
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
...
#[tauri::command]
async fn tables_list(node: tauri::State<'_, Arc<Node>>, space_id: Uuid) -> Result<Vec<Table>, String> {
    let spaces = node.spaces().clone();
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(&space_id).await.ok_or("space not found")?;
...
#[tauri::command]
async fn table_get(node: tauri::State<'_, Arc<Node>>, space_id: Uuid, table: &str) -> Result<Table, String> {
    let spaces = node.spaces().clone();
    let table_hash = Hash::from_str(table).map_err(|e| e.to_string())?;
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(&space_id).await.ok_or("space not found")?;
            space.tables().get_by_hash(table_hash).await.map_err(|e| e.to_string())
        })
    })
}

#[tauri::command]
async fn rows_query(node: tauri::State<'_, Arc<Node>>, space_id: Uuid, table: &str, offset: i64, limit: i64) -> Result<Vec<Row>, String> {
    let spaces = node.spaces().clone();
    let table_hash = Hash::from_str(table).map_err(|e| e.to_string())?;
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(&space_id).await.ok_or("space not found")?;
            space.rows().query(table_hash, String::from(""), offset, limit).await.map_err(|e| e.to_string())
        })
    })
}