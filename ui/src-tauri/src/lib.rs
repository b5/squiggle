...
use datalayer_node::space::tables::Table;
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .manage(Arc::new(node))
        .invoke_handler(tauri::generate_handler![spaces_list, events_search, users_list, programs_list, program_run, program_get, tables_list, table_get, rows_query])
...
#[tauri::command]
async fn tables_list(node: tauri::State<'_, Arc<Node>>, space: &str) -> Result<Vec<Table>, String> {
    let spaces = node.spaces().clone();
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
            let space = spaces.get(space).await.ok_or("space not found")?;
...
#[tauri::command]
async fn table_get(node: tauri::State<'_, Arc<Node>>, space: &str, table: &str) -> Result<Table, String> {
    let spaces = node.spaces().clone();
...
#[tauri::command]
async fn rows_query(node: tauri::State<'_, Arc<Node>>, space: &str, table: &str, offset: i64, limit: i64) -> Result<Vec<Row>, String> {
    let spaces = node.spaces().clone();
    let table_hash = Hash::from_str(table).map_err(|e| e.to_string())?;
    tokio::task::block_in_place(|| {
        tauri::async_runtime::block_on(async move {
